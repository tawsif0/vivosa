...
export default function MensCategoryPage({ category }) {
  ...
  useEffect(() => {
    const loadItems = async () => {
      try {
        const data = await fetchPublicMenApparel(category);
        setItems(data);
      } catch (error) {
        setItems([]);
      } finally {
      ...
      <section className="max-w-container-max mx-auto px-4 md:px-margin-desktop py-12 md:py-16">
        <div className="flex flex-col gap-4 max-w-3xl">
          ...
          <p className="font-body-lg text-on-surface-variant leading-relaxed">
            Browse our men&apos;s {categoryTitle.toLowerCase()} collection and premium fabrics.
          </p>
        </div>
      </section>
