...
export default function WomensPantsDetails() {
  const { id } = useParams();
  const item = useMemo(() => womensPants.find(p => p.id === id), [id]);
    ...
    <div className="bg-background text-on-background font-body-md text-body-md antialiased overflow-x-hidden pt-20">
        ...
        <div className="max-w-[1440px] mx-auto px-6 md:px-16 py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-[9px] tracking-[0.25em] uppercase text-neutral-400 font-light">
              PREMIUM PANT STYLE &amp; DESIGN
            </p>
          </div>
          <div className="text-[10px] tracking-[0.2em] uppercase text-neutral-400 font-light font-sans">
            <Link to="/" className="hover:text-white transition-colors">HOME</Link>
            <span className="mx-2 text-neutral-700">/</span>
            <Link to="/womens/pants" className="hover:text-white transition-colors">WOMEN&#39;S PANTS</Link>
            <span className="mx-2 text-neutral-700">/</span>
      ...
      <div className="w-full flex flex-col lg:flex-row min-h-[500px] border-b border-neutral-100">
        ...
        <div className="w-full lg:w-1/2 bg-[#f5f5f5] min-h-[420px] lg:min-h-[700px] overflow-hidden flex items-center justify-center p-8 md:p-12">
          <img src={item.image} alt={item.title} className="max-h-[540px] lg:max-h-[620px] w-full h-auto object-contain" loading="lazy" />
        </div>
        {/* Right content */}
          ...
          <div className="max-w-xl space-y-12">
            ...
            <div>
              ...
              <div className="text-sm font-sans text-neutral-700 leading-relaxed font-light space-y-2">
                {item.composition.map((line, idx) => (<p key={idx}>{line}</p>))}
              </div>
            </div>
	            <div>
	              ...
	              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
	                {item.detailImages.map((src, idx) => (
	                  <img key={idx} src={src} alt={`${item.title} color ${idx + 1}`} className="w-full h-auto max-h-24 object-contain border border-neutral-200 bg-white" loading="lazy" />
	                ))}
	              </div>
	            </div>
            <div className="pt-2 border-t border-neutral-100">
              ...
              <p className="mt-3 text-sm font-sans text-neutral-700 leading-relaxed font-light">
                Want more information about this product? Contact our team for MOQ, colors, and custom options.
              </p>
              <a href="/contact" className="mt-6 inline-flex items-center justify-center px-10 py-3 bg-[#1c1c1c] text-white font-semibold text-xs tracking-[0.25em] uppercase hover:bg-black transition-colors font-sans">
                Contact Us
              </a>
              <div className="mt-6">
                <Link className="text-xs font-sans text-neutral-400 hover:text-black tracking-[0.2em] uppercase transition-colors underline underline-offset-4" to="/womens/pants">
                  Back to Women's Pants
                </Link>
              </div>
            </div>
