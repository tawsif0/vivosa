...
const LeatherFootwearModify = () => {
  ...
  const loadItems = async () => {
    setLoading(true);
    try {
      const data = await fetchAdminSustainableLeathers("leather-footwear");
      setItems(data);
    } catch (error) {
      toast.error(error.response?.data?.error || "Failed to load footwear items");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (event) => {
    ...
    const loadingToast = toast.loading("Updating footwear leather item...");
    ...
    try {
      ...
      toast.success("Footwear leather item updated");
      handleCancelEdit();
      loadItems();
    } catch (error) {
      toast.dismiss(loadingToast);
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-bold text-slate-900 font-display">
            Modify Footwear Leather Items
          </h2>
          <p className="mt-1 text-sm text-slate-600">
            Manage or edit your footwear leathers here.
          </p>
        </div>
        <button
          onClick={loadItems}
          ...
          <form onSubmit={handleSubmit} className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="space-y-6">
              <ImageUploadBox
                ...
                inputId="footwear-modify-image-upload"
...
export default LeatherFootwearModify;
