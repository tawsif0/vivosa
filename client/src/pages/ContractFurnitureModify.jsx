...
const ContractFurnitureModify = () => {
  ...
  const loadItems = async () => {
    setLoading(true);
    try {
      const data = await fetchAdminSustainableLeathers("contract-furniture");
      setItems(data);
    } catch (error) {
      toast.error(error.response?.data?.error || "Failed to load furniture leathers");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (event) => {
    ...
    const loadingToast = toast.loading("Updating furniture leather item...");
    ...
    try {
      ...
      toast.success("Furniture leather item updated successfully");
      handleCancelEdit();
      loadItems();
    } catch (error) {
      toast.dismiss(loadingToast);
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-400">
            Sustainable Leather - Contract & Furniture
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-900 font-display">
            Modify Contract & Furniture Items
          </h2>
          <p className="mt-1 text-sm text-slate-600">
            Manage or edit your existing contract & furniture leathers here.
          </p>
        </div>
        <button
          onClick={loadItems}
      ...
      {editingItem ? (
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="space-y-6">
              <ImageUploadBox
                label="Leather Sample Image"
                description="Click to upload/replace sample image"
                preview={preview}
                fileName={imageFile?.name}
                inputId="furniture-modify-image-upload"
...
export default ContractFurnitureModify;
