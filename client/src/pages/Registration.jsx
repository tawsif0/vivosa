...
const Registration = () => {
  ...
  const { canRegister, loaded, settings } = usePublicSettings();
  ...
  useEffect(() => {
    if (loaded && !canRegister) {
      navigate("/login", { replace: true });
    }
  }, [canRegister, loaded, navigate]);
          ...
          <div className="p-7 md:p-8">
            <div className="text-center">
              <p className="mx-auto inline-flex rounded-full border border-deep-forest/10 bg-emerald-mint px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.28em] text-deep-forest">
                {settings?.isInitialSetup ? "Initial Setup" : "Account Creation"}
              </p>
              <h2 className="mt-4 text-4xl font-bold tracking-tight text-pure-black">
                {settings?.isInitialSetup ? "Create First Admin" : "Create Account"}
              </h2>
              <p className="mt-3 text-sm text-on-surface-variant">
                {settings?.isInitialSetup
                  ? "The first registered user will be set as the admin."
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-5">
              <div>
                ...
                <div className="relative">
                  <UserIcon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-on-surface-variant/50" />
                  <input
                    type="text"
                    {...register("name", {
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full rounded-2xl border bg-surface-bright px-12 py-3.5 pr-14 text-on-surface outline-none transition focus:border-deep-forest focus:ring-4 focus:ring-deep-forest/10 ${
                      errors.confirmPassword ? "border-red-400" : "border-outline-variant"
                    }`}
                    placeholder="••••••••"
                  />
                  <button
                ...
                {password && confirmPassword && password === confirmPassword ? (
                  <p className="mt-2 text-sm font-medium text-emerald-600">Passwords match</p>
                ) : null}
              </div>
              ...
              >
                {isLoading ? "Creating account..." : settings?.isInitialSetup ? "Create First Admin" : "Create Account"}
