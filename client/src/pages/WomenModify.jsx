...
const categoryNames = {
  ...
  shirts: "Shirts",
  "t-shirts": "T-shirts",
  "swim-lingerie": "Swim & Lingerie",
};

const categoryTitlePlaceholders = {
  sweater: "e.g. Oversized Knit Sweater",
  "jackets-coats": "e.g. Classic Trench Coat",
  pants: "e.g. High-Waisted Trousers",
  "polo-shirts": "e.g. Cotton Polo Shirt",
  shirts: "e.g. Silk Button-Down Shirt",
  "t-shirts": "e.g. Organic Cotton Tee",
...
const WomenModify = ({ category }) => {
  ...
  const loadItems = async () => {
    setLoading(true);
    try {
      const data = await fetchAdminWomenApparel(category);
      setItems(data);
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const updated = await updateWomenApparel(editingItem._id, payload);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteWomenApparel(deleteTarget._id);
      toast.dismiss(loadingToast);
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex items-center justify-between flex-wrap gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-400">
            Women &apos;s {categoryLabel}
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-900">Modify Women &apos;s {categoryLabel} Catalog</h2>
          <p className="mt-1 text-sm text-slate-600">
            Edit details, custom variants, or delete items from the women apparel database.
          </p>
        </div>
        <button
          onClick={loadItems}
          ...
          <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                      ...
                      {variantImagePreview ? (
                        ...
                        <div>
                          <input
                            ...
                            className="hidden"
                            id="women-variant-image-modifier"
                          />
                          <label
                            htmlFor="women-variant-image-modifier"
            ...
            <div className="space-y-6">
              <ImageUploadBox
                ...
                inputId="women-image-edit-upload"
