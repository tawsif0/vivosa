...
export default function MensPants() {
  ...
  useEffect(() => {
    const loadItems = async () => {
      try {
        const data = await fetchPublicMenApparel("pants");
        setItems(data);
      } catch (error) {
        setItems([]);
      } finally {
  ...
  return (
    <div className="bg-background text-on-background font-body-md text-body-md antialiased overflow-x-hidden pt-24">
      <section className="max-w-container-max mx-auto px-4 md:px-margin-desktop py-12 md:py-16">
        <div className="flex flex-col gap-4 max-w-3xl">
          ...
          <p className="font-body-lg text-on-surface-variant leading-relaxed">
            Browse our men&apos;s pants, trousers, and shorts collection.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-container-max mx-auto px-4 md:px-margin-desktop pb-20 md:pb-section-gap">
        {loading ? (
          <div className="text-center py-12 text-slate-500 font-sans">Loading pants...</div>
        ) : items.length === 0 ? (
          <div className="text-center py-12 text-slate-500 font-sans">No pants available yet.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-10 md:gap-x-gutter md:gap-y-12">
            {items.map((item) => (
              <Link
                key={item._id}
                to={`/mens/pants/${item._id}`}
